...
CVN_ss <- 15 #Sub Size Charpy Energy (Ft-lbs)

t_ss <- 6.67 # Sub size charpy thickness in mm
...
SMYS = sigy*1000 #n = 1e4 #NUmber of sims
## pipe specs ------------------------------------
...
## CorLas
The CorLas Model has two failure criteria that are compared against each other.  The first one is based on flow strength and the effective area of the flaw and the second one is based on the fracture toughness.

### Flow Stress - Effective Area Criterion  
The flow strength criterion is based on an effective area method similar to that used in corrosion assessment models and is based on the flow stress.  
...
Where, $0\ge C_f \le 1.0$. If $C_f = 0.5$ is used this will produce the
...
$a =$ `r paste(a, "in.")` (crack depth)

$L =$ `r paste(Lc,"in.")` (crack length)

$CVN =$ `r paste(round(CVN_us_fs,1), "Ft-lbs")` (Full-Size Upper Shelf Equivalent)
...
eps_y <- 0.005 - sigy / E #plastic strain @ 0.5% yield
...
$J_1c =$ `r paste(round(J1c,2), "(K*(ft-lb)/in^2")` 
