...
sigy <- 46 # ksi YS of steel
SMYS <- sigy * 1000 # Specified Minimum Yield Strength - convert to psi
K_1C <- 40 # ksi * in^1/2
...
# sigy # ksi - YS
sig_u <- 1.49 * sigy # ksi - UTS
# UTS ~ 1.49* YS on average if unknown

n_sh <- 0.537 - 0.526 * sigy / sig_u
...
a <- 0.25 # crack depth (in.)

Lc <- 5 # crack length (in.)
...
## CorLas
The CorLas Model has two failure criteria that are compared against each other.  The first one is based on flow strength and the effective area of the flaw and the second one is based on the fracture toughness J integral.

### Flow Stress - Effective Area Criterion  
The first criterion is based on an effective area method similar to that used in corrosion assessment models and is based on the flow stress.  
...
from 0.15 to 0.23. With higher yield strength steels having lower strain
hardening exponents and vice versa.

One such model for the strain hardening exponent is given in the CorLas Model documentation as the following: ^[Jaske, C.E. and Beavers, J.A., 2001, "Integrity and Remaining Life of Pipe with Stress Corrosion Cracking," PRCI Report, Catalog No. L51928]
...
the J-integral to the critical critical J and iterating the stress to
find where the two are equal. The J-integral is the sum of the elastic
...
### Shape Factor  
The shape factor, $Q_f$, is calculated from the following equation ^[ Jaske, C.E., 1989, "Estimation of the C* Integral for Creep-Crack-Growth Test Specimens," The Mechanism of Fracture, ASM International, Materials Park, OH, pp. 577-586]
...
The $f_3$ factor is given as the following equation. ^[ Shih, C.F. and Hutchinson, J.W., 1975, "Fully Plastic Solutions and Large Scale Yielding Estimates for Plane Stress Crack Problems," Report No. DEAP S-14, Harvard University, Cambridge, MA]
$$
f_3(n) = \left[3.85\left(\frac{1}{n}\right)^{\frac{1}{2}}\left(1-n\right)+\pi n\right]\left(1+n\right)
$$
If the J fracture toughness data is not available for the material under
consideration, the critical fracture toughness can be estimated from the
upper shelf Charpy V-Notch (CVN) values. Some empirical relationships
...
eps_y <- 0.005 - sigy / E # plastic strain @ 0.5% strain

# Material Strength Factor
Ks <- (sigy / (0.005 - sigy / E) ^ n_sh) # eq(14) in IPC paper
...
J <-  function(S) {
  Q_f * F_s * a * (S ^ 2 * pi  / E + f_3 * n_sh * (S / Ks) ^ (1 / n_sh)  * S) - J1c
}
...
The two failure stress criterion are calculated and the minimum of the two are selected.
...
$\% SMYS =$ `r paste(round(fail/sigy,3)*100,"%")`
